...
PyObject* get_2darray(const std::vector<::std::vector<Numeric>>& v)
{
    ...
    PyArrayObject *varray =
        unsafe_cast<PyArrayObject *>(PyArray_SimpleNew(2, vsize, NPY_DOUBLE));
...
{
  auto sz = static_cast<Py_ssize_t>(strings.size());
  PyObject* list = PyList_New(sz);
  for (Py_ssize_t i = 0; i < sz; ++i) {
    PyList_SetItem(list, i, PyString_FromString(strings[static_cast<std::size_t>(i)].c_str()));
  }
  return list;
}
...
inline void colorbar(PyObject* mappable = NULL, const std::map<std::string, float>& keywords = {})
{
    ...
    {
        PyDict_SetItemString(kwargs, it->first.c_str(), PyFloat_FromDouble( static_cast<double>(it->second)) );
    }
...
inline void xticks(const std::vector<Numeric> &ticks, const std::vector<std::string> &labels = {}, const std::map<std::string, std::string>& keywords = {})
{
    ...
    if(labels.size() == 0) {
        ...
        PyObject* labelstuple = PyTuple_New(static_cast<Py_ssize_t>(labels.size()));
        for (size_t i = 0; i < labels.size(); i++)
            PyTuple_SetItem(labelstuple, static_cast<Py_ssize_t>(i), PyUnicode_FromString(labels[i].c_str()));
...
inline std::vector<std::array<double, 2>> ginput(const int numClicks = 1, const std::map<std::string, std::string>& keywords = {})
{
    ...
    const size_t len = static_cast<std::size_t>(PyList_Size(res));
